...
#' @param p the coastal points as a `sp::SpatialPoints` object.
...
autoDetect3 <- function(x, threshold = 2, val="sst", 
                        p=sample_points$km100, 
                        smooth.method = "ksmooth",
                        l.offshore=buffer300$wintri$line,
                        d.offshore=50, 
                        l.coast=buffer20$wintri$line,
                        d.coast=0, 
                        na.rm=TRUE, ...) {
  ...
  r <- x %>% dplyr::select(lon, lat, val) %>% raster::rasterFromXYZ(crs="+proj=longlat")
  r <- raster::projectRaster(r, crs=crs(p))
  ...
  df <- data.frame(coast.pts@coords, offshore=off.sst, coast=coast.sst, 
                   upwelling=(off.sst-coast.sst)>=threshold)
  colnames(df) <- c("x", "y", "offshore.SST", "coast.SST", "upwelling")
  
  return(list(df=df, offshore.pts=offshore.pts, coast.pts=coast.pts,
